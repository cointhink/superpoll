'use strict'
//var pgp = require('pg-promise')({query: x=>console.log(x.query)})
var pgp = require('pg-promise')()
var http = require('request-promise')
var uuid = require('uuid')

module.exports = new (function(){
  let that = this
  let db

  this.setup = function(config) {
    db = pgp(config.cockroach)
    return ensureDb('cointhink')
      .then(ensureTables)

  }

  function ensureDb(name) {
    return db.query('SHOW DATABASES')
    .then(dbs => {
      let names = dbs.map(db=>db.Database)
      if (names.indexOf(name) == -1) {
        console.log('warning: creating database', name)
        return db.query('CREATE DATABASE '+name)
      }
    })
  }

  function ensureTables() {
    return tableTest('exchanges', `
        (
         id varchar(128) PRIMARY KEY,
         name varchar(128),
         marketsUrl text,
         marketsJs text,
         orderbookJs text,
         offerJs text
        )
      `)
      .then(() => tableTest('orderbooks', `
        (
         id varchar(128) PRIMARY KEY,
         date timestamp,
         exchangeId varchar(128),
         base varchar(128),
         quote varchar(128)
        )
      `))
      .then(() => tableTest('offers', `
        (
         id varchar(128) PRIMARY KEY,
         orderbookId varchar(128),
         bidAsk varchar(128),
         price numeric(20,10),
         quantity numeric(20,10)
        )
      `))
      .then(() => indexTest('ByOrderBookAndBidAsk',
                            'offers (orderbookId, bidAsk)'))
  }

  function tableTest(name, schema) {
    return db.query('SHOW TABLES')
    .then(tables => {
      let names = tables.map(t=>t.Table)
      if(names.indexOf(name) == -1) {
        console.log('** warning: creating table', name)
        return db.query('CREATE TABLE '+name+' '+schema)
      }})
  }

  function indexTest(name, index) {
    let sql = 'CREATE INDEX IF NOT EXISTS '+name+' ON '+index
    return db.query(sql)
  }

  this.exchanges = function() {
    return db.query('SELECT * FROM exchanges')
  }

  this.marketlist = function(exchange) {
    let markets_format = jseval(exchange.marketsJs)
    var req = {method: 'get', url: exchange.marketsUrl, json: true, timeout: 5000}
    return http(req)
      .then(function(body) {
        exchange.orderbooks = markets_format(body)
        exchange.orderbooks.forEach(function(ob){ob.exchange = exchange.id})
        return exchange
      }, err => console.log('marketlist err', req.url, err.message))
  }

  this.jsput = function(exchange_name, part, js) {
    let insert = "INSERT into exchanges values ($1, $2)"
    return db.query(insert, [exchange_name, exchange_name])
      .then(() => console.log('warning exchange', exchange_name, 'created'),
            () => console.log('updating', exchange_name, part))
      .then(() => {
        let sql = "UPDATE exchanges set $1~=$2 where name=$3"
        let params = [part, js, exchange_name]
        return db.query(sql, params)
      })
  }

  this.poll = function(exchange, market) {
    market.date = new Date()
    let orderbook
    if(market.bids) {
      orderbook = Promise.resolve({bids: market.bids, asks: market.asks})
    } else {
      let req = {method: 'get', url: market.url, json: true, timeout: 5000}
      orderbook = http(req)
    }

    let orderbook_format = jseval(exchange.orderbookJs)
    let offer_format = jseval(exchange.offerJs)
    return orderbook.then(function(ob) {
      let obook = orderbook_format(ob)
      if(obook.bids && obook.asks) {
        market.bids = obook.bids.map(function(ofr) {return offer_format(ofr)})
        market.asks = obook.asks.map(function(ofr) {return offer_format(ofr)})
        return market
      }
    }, function(err){
      //console.log('poll err', err.message, err.options.url)
    })
  }

  this.insert = function(orderbook) {
    //console.log('insert', orderbook)
    let sql = 'INSERT INTO orderbooks (id, "exchangeId", date, base, quote) '+
              'values (${uuid}, ${eid}, ${date}, ${base}, ${quote})'
    let oid = newId('orderbooks')
    let params = {uuid: oid, eid: orderbook.exchange, date: orderbook.date.toISOString(),
                  base: orderbook.market.base, quote: orderbook.market.quote}
    return db.query(sql, params).then(result => {
      // todo: order the offers
      let asks = Promise.all(orderbook.asks.map(ask => this.insertOffer(oid, 'ask', ask[0], ask[1])))
      let bids = Promise.all(orderbook.bids.map(bid => this.insertOffer(oid, 'bid', bid[0], bid[1])))
      return Promise.all([asks, bids]).then(x=>orderbook)
    }, e => console.log(e))
  }

  this.insertOffer = function(orderbookId, bidAsk, price, quantity) {
    let sql = 'INSERT INTO offers (id, "orderbookId", bidAsk, price, quantity) '+
              'values (${uuid}, ${oid}, ${bidask}, ${price}, ${quantity})'
    let params = {uuid: newId('offers'), oid: orderbookId, bidask: bidAsk, price: parseFloat(price),
                  quantity: quantity}
    return db.query(sql, params)
  }

  this.last = function(market, now, duration) {
    let sql = 'SELECT * from orderbooks where exchangeId = ${exchange} ' +
              'and base = ${base} and quote = ${quote} ' +
              'and date > ${time}::timestamp'
    let time = new Date(now - duration)
    let params = {exchange: market.exchange,
                  base: market.market.base,
                  quote: market.market.quote,
                  time:time}
    return db.query(sql, params).catch(x=>console.log(x))
  }

  this.stop = function() {
    return pgp.end()
  }

  function jseval(jsfunc) {
    let js = '('+jsfunc+')'
    return eval(js)
  }

  function newId(tableName) {
    return tableName+uuid().substr(18,24)
  }

})()
