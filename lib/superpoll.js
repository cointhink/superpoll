                ...
                rethink.tableList().run(conn)
                  .then(function(tables){
                    ...
                    if(tables.indexOf('orderbooks') == -1) {
                      ...
                      return rethink.tableCreate('orderbooks').run(this.conn)
                    }
                  })
                  .then(function(){
                    return rethink.table('orderbooks').indexList().run(conn)
                  })
                  .then(function(indexes){
                    console.log(indexes)
                    if(indexes.indexOf('exchange') == -1) {
                      console.log('creating index exchange')
                      return rethink.table('orderbooks')
                          .indexCreate('exchange', rethink.row("exchange")).run(conn)
                    }
                  })
              })
  }
